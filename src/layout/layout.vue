<template>
  <div class="app-admin">
    <LeftSlider :current-page="currentPage"
                :menu-hidden="menuHidden"
                :is-webkit="isWebkit"
                @openPage="openPage" />
    <div :class="{'right-content':true, 'right-content100':menuHidden}">
      <RightContentHeader :nav-open-list="openNavList"
      ...
      <page-view :header-hidden="headerHidden"
                 :is-reload="isReload"
...
export default {
  ...
  mounted() {
    ...
    this.$route.matched[1] && this.$route.matched[1].components && this.addRouteCache(this.$route.matched[1].components.default.name)
